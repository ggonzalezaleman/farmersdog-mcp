...
function saveSession(wsEndpoint, token, expiresAt) {
    try {
        const data = { wsEndpoint, token, tokenExpiresAt: expiresAt, savedAt: Date.now() };
        writeFileSync(SESSION_FILE, JSON.stringify(data, null, 2));
        log("Session saved to disk");
    }
    catch (e) {
        log(`Failed to save session: ${e.message}`);
    }
}
...
class FarmersDogClient {
    ...
    async getAuthenticatedPage() {
        ...
        if (!this.email || !this.password || !this.browserbaseApiKey || !this.browserbaseProjectId) {
            throw new Error("Missing credentials. Set FARMERSDOG_EMAIL, FARMERSDOG_PASSWORD, BROWSERBASE_API_KEY, BROWSERBASE_PROJECT_ID");
        }
        for (let attempt = 1; attempt <= MAX_LOGIN_RETRIES; attempt++) {
            ...
            try {
                ...
                if (attempt === MAX_LOGIN_RETRIES) {
                    throw new Error(`Login failed after ${MAX_LOGIN_RETRIES} attempts. Cloudflare Turnstile may be blocking. ` +
    ...
    async query(queryStr, variables = {}, retry = true) {
        ...
        try {
            const result = await page.evaluate(async ({ url, query, variables, token }) => {
                const fetchFn = window.__cleanFetch || window.fetch;
                ...
                try {
                    ...
                    try {
                        ...
                        return { error: `Non-JSON response (${r.status}): ${text.substring(0, 200)}` };
                    }
                }
                catch (e) {
            ...
            if (retry && e.message?.includes("Target closed") || e.message?.includes("Session closed")) {
                log("Session died mid-query, reconnecting...");
                clearSavedSession();
                return this.query(queryStr, variables, false);
            }
            throw e;
        }
    }
    async queryCustomer(queryStr, variables = {}, retry = true) {
        ...
        try {
            ...
            if (retry && (e.message?.includes("Target closed") || e.message?.includes("Session closed"))) {
                log("Session died mid-query, reconnecting...");
                clearSavedSession();
                return this.queryCustomer(queryStr, variables, false);
            }
            throw e;
        }
    }
    ...
    async rescheduleOrder(subscriptionId, newDate) {
        return this.query(`mutation RescheduleNextOrder($input: RescheduleNextOrderInput!) {
        rescheduleNextOrder(input: $input) { success }
      }`, { input: { subscriptionId, newDate } });
    }
    async updatePet(petId, input) {
        return this.query(`mutation EditPetSubmit($petId: Int!, $input: UpdatePetInput!) {
    ...
    async quoteRecipeChange(petId, recipes) {
        return this.queryCustomer(`query FetchChangeFreshRecipesQuote($input: ChangeFreshRecipesPlanQuoteInput!) {
    ...
    async updateOrderSize(orderSize) {
        return this.queryCustomer(`mutation UpdateOrderSize($input: ChangeFreshOrderSizeInput!) {
    ...
    async updateRecipes(planId, recipes) {
        return this.queryCustomer(`mutation UpdateFoodPlansRecipes($input: UpdateFoodPlansRecipesInput!) {
...
function toolHandler(fn) {
    return async (...args) => {
        try {
            ...
            return {
                content: [{ type: "text", text: `Error: ${error.message}. Try again — if auth fails repeatedly, set FARMERSDOG_TOKEN manually.` }],
                isError: true,
            };
        }
    };
...
async function main() {
    ...
    // ── Read-only tools ──
    server.tool("farmersdog_get_account", "Get Farmer's Dog account overview: pets, subscriptions, recent orders, referral code", {}, toolHandler(() => client.getAccount()));
    server.tool("farmersdog_next_delivery", "Get next scheduled delivery date and subscription details", {}, toolHandler(() => client.getNextDelivery()));
    server.tool("farmersdog_delivery_history", "Get past delivery history (shipped orders)", { limit: z.number().optional().default(10).describe("Number of past deliveries to retrieve") }, toolHandler(({ limit }) => client.getDeliveryHistory(limit)));
    server.tool("farmersdog_get_pets", "Get registered pets (active and inactive)", {}, toolHandler(() => client.getPets()));
    server.tool("farmersdog_get_pet_details", "Get detailed pet info: weight, activity, calories, breeds", {}, toolHandler(() => client.getPetDetails()));
    server.tool("farmersdog_available_dates", "Get available dates for rescheduling next delivery. Call this BEFORE reschedule_delivery.", {}, toolHandler(() => client.getAvailableDates()));
    server.tool("farmersdog_get_profile", "Get customer profile (name, email)", {}, toolHandler(() => client.getProfile()));
    server.tool("farmersdog_get_orders", "Get current + future + past orders with pricing, shipping, and delivery details", {}, toolHandler(() => client.getOrders()));
    server.tool("farmersdog_list_recipes", "List all available Farmer's Dog recipes (TURKEY, BEEF, CHICKEN, PORK, CHICKEN_OATS_COLLARDS)", {}, toolHandler(() => client.listAvailableRecipes()));
    server.tool("farmersdog_get_recipes", "Get current recipes for your pets with pricing. Use plan.id for update_recipes.", {}, toolHandler(() => client.getRecipes()));
    server.tool("farmersdog_quote_recipe_change", "Get price quote for changing recipes BEFORE confirming. Shows price diff and new frequency.", {
    ...
    }, toolHandler(({ petId, recipes }) => client.quoteRecipeChange(petId, recipes)));
    server.tool("farmersdog_get_order_size_quotes", "Get available order sizes with pricing comparison (current vs min vs max)", {}, toolHandler(() => client.getOrderSizeQuotes()));
    // ── Mutating tools ──
    server.tool("farmersdog_reschedule_delivery", "Reschedule next delivery via API. Call farmersdog_available_dates first to get valid dates.", {
    ...
    }, toolHandler(({ subscriptionId, newDate }) => client.rescheduleOrder(subscriptionId, newDate)));
    server.tool("farmersdog_reschedule_delivery_ui", "Reschedule via website UI (more reliable, handles Cloudflare). Pass week label exactly as shown.", {
    ...
    }, toolHandler(({ targetWeek, reason }) => client.rescheduleViaUI(targetWeek, reason)));
    server.tool("farmersdog_update_pet", "Update pet info (birthday, weight, activity, condition)", {
    ...
    }, toolHandler(({ petId, birthday, weight, targetWeight, activity, condition }) => {
        const input = {};
        if (birthday) {
    ...
    server.tool("farmersdog_update_recipes", "Confirm recipe changes. Call farmersdog_quote_recipe_change first to see pricing impact.", {
        planId: z.number().describe("Plan ID (from farmersdog_get_recipes -> plan.id)"),
        recipes: z.array(z.object({
            name: z.string().describe("Recipe code: TURKEY, BEEF, CHICKEN, PORK, or CHICKEN_OATS_COLLARDS"),
        })).describe("1-3 recipes to set"),
    }, toolHandler(({ planId, recipes }) => client.updateRecipes(planId, recipes)));
    server.tool("farmersdog_update_order_size", "Change order size/frequency. 28 = 4 weeks, 56 = 8 weeks. Larger = cheaper per day.", {
        orderSize: z.number().describe("Order size in days (28 or 56)"),
    }, toolHandler(({ orderSize }) => client.updateOrderSize(orderSize)));
    // Cleanup on exit
